...
class CategoryForObjectNode(template.Node):
    ...
    def render(self, context):
        context[self.context_var] = \
            self.obj.resolve(context).categories.all()
        return ''
...
def get_category_list(parser, token):
    """
    Gets the categories for the specific Post object.

    Syntax::

        {% get_category_list for object as [var_name] %}

    Example usage::

        {% get_category_list for object as category_list %}
    """
    bits = token.contents.split()
    if len(bits) != 5:
        raise template.TemplateSyntaxError, "%s tag requires 5 arguments" % bits[0]
    if bits[1] != 'for':
        raise template.TemplateSyntaxError, "second argument to %s tag must be 'for'" % bits[0]
    if bits[3] != 'as':
        raise template.TemplateSyntaxError, "fourth argument to %s tag must be 'as'" % bits[0]

    return CategoryForObjectNode(bits[2], bits[4])
